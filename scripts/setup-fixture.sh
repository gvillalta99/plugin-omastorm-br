...
# that the engine embeds at build time and the Natural Earth files that
...
# populated places), then verifies data/SHA256SUMS. data/raw is ignored, so a
# fresh checkout runs this once before `cargo build`. Launch never calls it.
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p data/raw
