#!/usr/bin/env bash
# One continuous live take of the window for the announcement, 1280×720 at
...
ffmpeg -hide_banner -loglevel error -y -framerate 30 -i "$demo_dir/frames/%04d.png" \
  -c:v libx264 -preset slow -crf 19 -pix_fmt yuv420p -movflags +faststart docs/media/omastorm-live-demo.mp4
ffprobe -v error -show_entries stream=width,height,r_frame_rate,nb_frames:format=duration,size -of json docs/media/omastorm-live-demo.mp4 > review/demo-live-validation.json
grep -h "^Live\|backfilled [0-9]" "$XDG_RUNTIME_DIR/omastorm/engine.log" | sed 's/^/  engine: /' | head -6
echo "docs/media/omastorm-live-demo.mp4"
